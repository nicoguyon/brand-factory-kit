...
histoire, lookbook, cabine d'essayage live (styliste IA + Nano Banana, tout via fal), FAQ, film, JSON-LD, llms.txt.
...
(OUT / "api" / "_brand.json").write_text(json.dumps({
    ...
    "sizes": B.get("sizes", []), "size_question": B.get("size_question", ""), "stylist_model": T.get("stylist_model", "google/gemini-2.5-flash"),
...
async function ask(){const text=$('#talk').value.trim();if(!text)return toast('Décris ce que tu aimes et comment tu l\\'utiliserais.');$('#verdict').textContent='Le styliste réfléchit…';try{const r=await fetch('/api/stylist',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});const j=await r.json();if(!r.ok)throw new Error(j.error);const p=P.find(x=>x.id===j.variante);if(j.taille)size=j.taille;setPick(p.id);const pr=j.probabilites||{};
$('#verdict').innerHTML=`<div class="small">Ton styliste a choisi :</div><div class="big">${p.name}${j.taille?' · '+j.taille:''}</div><p class="small" style="margin:6px 0">« ${j.raison||''} »</p><div class="small">Décidé en ${(j.ms/1000).toFixed(1)} s</div><div class="bars">${Object.entries(pr).sort((a,b)=>b[1]-a[1]).filter(([k])=>P.find(x=>x.id===k)).map(([k,v])=>`<div><span>${P.find(x=>x.id===k).name}</span><i style="width:${Math.round(v*140)}px"></i>${Math.round(v*100)} %</div>`).join('')}</div>`;if(photo)tryon()}catch(e){$('#verdict').textContent='Styliste indisponible : '+e.message}}
$('#ask').onclick=ask;
...
<div class="step"><b>2</b><h4>Ton styliste <span class="jev">IA</span></h4><p class="small">Dis-lui ce que tu aimes et comment tu l’utiliserais : il choisit pour toi et t’explique pourquoi.</p><div class="chips" id="chips"></div><textarea id="talk" rows="3" placeholder="Ou écris avec tes mots…"></textarea><div class="row"><button class="opt" id="mic">🎙 Dicter</button><button class="opt" id="ask">Demander au styliste</button><button class="opt" id="surprise">🎲 Surprends-moi</button></div><div class="verdict" id="verdict"></div><p class="small">Ou choisis toi-même :</p><div class="mini" id="mini"></div></div>
<div class="step"><b>3</b><h4>Le résultat</h4><div class="shot"><img id="res" alt=""><span id="resph">Le résultat apparaîtra ici</span><div class="spin" id="spin"></div></div><button class="cta" id="go" style="width:100%;margin-top:12px" disabled>Essayer</button><div class="note" id="tnote">Styliste : modèle de langage via fal. Rendu : Nano Banana 2 via fal. La photo n'est pas conservée.</div></div></div></div></section>""" if T else ""
